...
**Primary model - XGBoost.** An XGBoost classifier is selected as the primary model for its ability to capture non-linear interactions between financial ratios and qualitative flags without requiring explicit feature crosses. The `scale_pos_weight` parameter is set to the observed class imbalance ratio (4.75) so the boosting process assigns proportionally higher loss to missed defaults. Hyperparameters are selected via 5-fold stratified grid search optimising Average Precision (PR-AUC). ROC-AUC is not inherently broken under class imbalance; it measures rank separation between distributions and is invariant to class proportions. PR-AUC is chosen here for a different reason: it focuses the search on the precision-recall trade-off for the positive class, which directly mirrors the downstream objective of maximising recall at a tuned threshold. Candidates that score well on PR-AUC tend to have more headroom to trade precision for recall when the threshold is lowered, which is exactly the operating mode used in production.
